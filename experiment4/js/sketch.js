...
var myp5 = new p5(f, 'canvasContainerTwo');
...
var q = function( sketch ) {
  ...
  sketch.setup = function() {
    ...
    let label = sketch.createP();
    label.html("Lily World Key: ");
    label.parent("labelContainerThree");
  ...
  let [tw, th] = [sketch.p3_tileWidth(), sketch.p3_tileHeight()];

  sketch.p3_drawTile = function(i, j) {
    sketch.XXH = XXH;
    colors = colors = [
      sketch.color(0,76,57), 
      sketch.color(0,112,84), 
      sketch.color(4,144,109), 
      sketch.color(0,61,46)
    ];
    let tileType = XXH.h32("tile:" + [i, j], worldSeed) % 2;  // Allow types 0–3
    let currentColor = colors[tileType % colors.length];
    sketch.push();
    // translate(this.x, this.y);
    sketch.angleMode(sketch.DEGREES);
    ...
    if (tileType == 0) {
      // Shape 1
      //sketch.rotate(-45);
      sketch.arc(0, 0, tw * 4, th * 4, 0, 90);  
      // Bottom-right corner
      
      sketch.arc(tw * 2, tw * 2, tw * 4, th * 4, 180, 270);
      
      // arc(0, 0, tw, tw, 0, 90);
      // arc(tw, tw, tw, tw, 180, 270);
    } else {
      // Shape 2
      sketch.arc(tw * 2, 0, tw * 4, th * 4, 90, 180); 
      // Bottom-left corner
      sketch.arc(0, th * 2, tw * 4, th * 4, 270, 360);
      // arc(tw, 0, tw, tw, 90, 180);
      // arc(0, tw, tw, tw, 270, 360);
    }
    /*
    let n = clicks[[i, j]] | 0;
    if (n % 2 == 1) {
...
var myp5 = new p5(q, 'canvasContainerThree');
